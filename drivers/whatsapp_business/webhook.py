...
Handles Meta Cloud API webhook verification (GET) and message processing (POST).
Called from modules/messaging/webhooks/router.py.
...
async def verify_webhook(request: Request, account_id: str) -> PlainTextResponse:
    ...
    if mode == "subscribe" and challenge:
        # Verify token must match the one configured in Meta Business Manager.
        # We look it up from MessagingSettings for the account.
...
async def _get_verify_token(account_id: str) -> str:
    ...
    try:
        ...
        # In the current architecture, verify token lives in MessagingSettings.
        # For now, fall back to a generic env var; could be per-account in future.
